"""
shap_audit.py — Phase 8: Portfolio-Wide Model Interpretation
============================================================
- Logic: Uses SHAP TreeExplainer to attribute alpha sources for all Meta-Models.
...
def run_audit(asset):
    ...
    if not os.path.exists(model_path):
        print(f"   [SKIP] {asset.upper()} — model not found at {model_path}")
        return

    print(f"\n{'='*60}")
    print(f"  ASSET: {asset.upper()} (SHAP Portfolio Audit)")
    print(f"{'='*60}")
    ...
    # 4. Global Importance (Mean Absolute SHAP)
