...
def evaluate_d(assets,thresh):
    ...
    for name, df in assets:
        ...
        ax1.set_xlabel('d (Differentiation Order)')
        ...
        ax1.set_xticklabels([f"{v:.2f}" for v in d_values], rotation=45) 
        ax1.grid(True, which="both", axis='x', ls="--", alpha=0.4)
        ...
        ax2.plot(d_values, corrs, marker='x', linestyle='--', color='tab:green', label='Correlation (Memory)')
